	...
	<div class="dr_tabs-wrapper">
		...
		<nav class="dr_tab-nav">
			...
			<div class="dr_swiper-navigation">
				<div id="dr_swiper-prev-<?php echo esc_attr( $block_id ); ?>" class="dr-swiper-prev dr-swiper-arrow"></div>
				<div id="dr_swiper-next-<?php echo esc_attr( $block_id ); ?>" class="dr-swiper-next dr-swiper-arrow"></div>
			</div>
		</nav>
	</div>
	<div class="dr_tabs-content-wrapper">
		...
		if ( $all_recipes_query->have_posts() ) {
			...
			<div id="dr_tab-content-all-<?php echo esc_attr( $block_id ); ?>" class="dr_tab-content dr_active">
				<div data-swiper-options="<?php echo esc_attr( wp_json_encode( $slider_settings ) ); ?>" class="dr_recipe-slider-tab-2 swiper swiper-container" data-id="<?php echo esc_attr( $block_id ); ?>">
					<div class="swiper-wrapper">
						...
						while ( $all_recipes_query->have_posts() ) {
							...
							dr_widgets_blocks_get_template(
								'recipe-categories-tab-content-two.php',
								$data,
								'',
								DR_WIDGETS_BLOCKS_PLUGIN_PATH . '/templates/recipe-cat-tab-two/'
							);
							$i++;
						}
						?>
