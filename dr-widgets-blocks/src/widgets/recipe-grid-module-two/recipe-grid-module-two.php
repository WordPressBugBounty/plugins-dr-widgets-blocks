...
class Widget_Recipe_Grid_Module_Two extends Widget {
	/**
	 * @var $widget_name
	 */
	public $widget_name = 'dr-recipe-grid-module-two';

	public function get_title() {
		return esc_html__( 'Recipe Grid Module 2', 'dr-widgets-blocks' );
	}
	...
	public function get_style_depends()	{
		...
		wp_register_style( 'dr-widgetsBlocks-recipe-grid-module-two', plugin_dir_url( DR_WIDGETS_BLOCKS_PLUGIN_FILE ) . 'assets/build/recipeGridModuleTwo.css' );

		return array(
			'dr-widgetsBlocks-layouts',
			'dr-widgetsBlocks-recipe-grid-module-two',
		);
	}
	protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .dr-grid-2' => '--column-gap: {{SIZE}}{{UNIT}};',
				),
			)
		);
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .dr-grid-2' => '--row-gap: {{SIZE}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_responsive_control(
			'maxHeight',
			array(
				'label'      => esc_html__( 'Max Height', 'dr-widgets-blocks' ),
				'type'       => \Elementor\Controls_Manager::SLIDER,
				...
				'selectors'  => array(
					'{{WRAPPER}} .dr-grid-2--layout-2' => '--max-height: {{SIZE}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_group_control(
			...
			array(
				...
				'selector' => '{{WRAPPER}} .dr-grid-2__recipe',
			)
		);
		$this->add_control(
			'borderRadius',
			array(
				...
				'size_units' => array( 'px', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .dr-grid-2__recipe, {{WRAPPER}} .dr-widgetBlock_fig-wrap' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		$this->add_group_control(
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .dr-grid-2__recipe , {{WRAPPER}} .dr-widget .dr-widgetBlock_content-wrap' => 'text-align: {{VALUE}};',
					'{{WRAPPER}} .dr-grid-2__recipe .dr_meta-items' => 'justify-content: {{VALUE}};',
				),
			)
		);
		$this->end_controls_section();
        ...
        $this->add_group_control(
            ...
            array(
                ...
                'selector' => '{{WRAPPER}} .dr-grid-2__recipe--hero .dr_title',
            )
        );
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .dr-grid-2__recipe--hero .dr-widgetBlock_fig-wrap img' => 'width: {{SIZE}}{{UNIT}};',
				),
			)
		);
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .dr-grid-2__recipe--hero .dr-widgetBlock_fig-wrap img' => 'height: {{SIZE}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .dr-grid-2__recipe--hero .dr-widgetBlock_fig-wrap img' => 'object-fit: {{VALUE}};',
				),
				'default'   => 'cover',
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .dr-grid-2__recipe--regular .dr-widgetBlock_fig-wrap img' => 'width: {{SIZE}}{{UNIT}};',
				),
			)
		);
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .dr-grid-2__recipe--regular .dr-widgetBlock_fig-wrap img' => 'height: {{SIZE}}{{UNIT}};',
				),
			)
		);
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .dr-grid-2__recipe--regular .dr-widgetBlock_fig-wrap img' => 'object-fit: {{VALUE}};',
				),
				'default'   => 'cover',
			)
		);
		...
		$this->add_group_control(
			...
			array(
				...
				'selector' => '{{WRAPPER}} .dr-grid-2__recipe--regular .dr_title',
			)
		);
		$this->end_controls_section();
		...
		$this->add_control(
			...
			array(
				...
				'type'      => \Elementor\Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .dr-grid-2__recipe .dr_meta-content .dr_meta-item' => 'color: {{VALUE}};',
				),
			)
		);
		$this->add_responsive_control(
	...
	protected function render() {
		...
		<div class="dr-widget">
			<div class="dr-grid-2 dr-grid-2--layout-<?php echo esc_attr($layout_number); ?>">
				<?php
				$i = 0; 
				if ( $recipes_query->have_posts() ) {
					while ( $recipes_query->have_posts() ) {
						...
						dr_widgets_blocks_get_template( 'recipes-grid-module-two.php', $data );
						$i++;
					}
					wp_reset_postdata();
				}
