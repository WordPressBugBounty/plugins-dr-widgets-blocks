...
(function ($) {
    jQuery(window).on('elementor/frontend/init', () => {
        class PaginationHandler extends elementorModules.frontend.handlers.Base {
            ...
            initPagination() {
                let containerID = document.querySelectorAll('.elementor-widget-dr-recipe-grid-two');
            ...
            fetchRecipes(dataSettings, taxonomy, terms, pagedValue, elementID, append, callback = () => {}) {
                ...
                const $recipeGrid = $container.find('.dr-widgetBlock_row');
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        action: 'dr_widgets_blocks_recipe_pagination_two',
                        nonce: recipeGridTwo.nonce,
                        paged: pagedValue,
                        ...
                        recipesPerRow: dataSettings['recipesPerRow'],
                        recipesPerRow_tablet: dataSettings['recipesPerRow_tablet'],
                        recipesPerRow_mobile: dataSettings['recipesPerRow_mobile'],
                        showPagination: dataSettings['showPagination'],
                        paginationType: dataSettings['paginationType'],
                        showFeatureImage: dataSettings['showFeatureImage'],
                        imageSize: dataSettings['imageSize'] ?? 'delrecpe-structured-data-4_3',
                        imageSizel2: dataSettings['imageSizel2'] ?? 'delrecpe-structured-data-1_1',
                        ...
                        showAuthor: dataSettings['showAuthor'] ?? 'no',
                        showPublishDate: dataSettings['showPublishDate'] ?? 'no',
                    ...
                    success: function (response) {
                        ...
                        if (response.success ?? true) {
                            if (append) {
                                ...
                                // Find all .dr_column elements
                                const columns = tempDiv.querySelectorAll('.dr_column');
                                // Append only the .dr_column elements
                                columns.forEach(column => {
                                    $recipeGrid.append(column);
                                });
                            } else {
        ...
        elementorFrontend.elementsHandler.attachHandler('dr-recipe-grid-two', PaginationHandler);
    });
})(jQuery);