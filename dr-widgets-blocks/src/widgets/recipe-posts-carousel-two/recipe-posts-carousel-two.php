...
class Widget_Recipe_Posts_Carousel_Two extends Widget {
	/**
	 * @var $widget_name
	 */
	public $widget_name = 'dr-recipe-posts-carousel-two';

	public function get_title() {
		return esc_html__( 'Recipe Posts Carousel 2', 'dr-widgets-blocks' );
	}
	...
	public function get_style_depends()	{
		...
		wp_register_style( 'dr-widgetsBlocks-recipe-posts-carousel-two', plugin_dir_url( DR_WIDGETS_BLOCKS_PLUGIN_FILE ) . 'assets/build/recipePostsCarouselTwo.css' );

		return array(
			'dr-widgetsBlocks-layouts',
			'dr-widgetsBlocks-recipe-posts-carousel-two',
			'swiper',
			'e-swiper',
		);
	}
	...
	public function get_script_depends()	{
		wp_register_script( 'drWidgets-posts-carousel-two', plugin_dir_url( DR_WIDGETS_BLOCKS_PLUGIN_FILE ) . 'src/widgets/recipe-posts-carousel-two/recipe-posts-carousel-two.js', array( 'jquery' ), DR_WIDGETS_BLOCKS_VERSION, true );

		return array(
			'drWidgets-posts-carousel-two',
			'swiper-bundle'
		);
	}
	...
	protected function register_controls() {
		/**
		 * Recipe Posts Carousel Two Layouts Section
		 */
		...
		/**
		 * Recipe Posts Carousel Two Query Section
		 */
		$this->start_controls_section(
			'query_section',
			array(
		...
		/**
		 * Recipe Posts Carousel Two Slider Section
		 */
		$this->start_controls_section(
			'slider_section',
			array(
		...
		/**
		 * Recipe Posts Carousel Two Additional Section
		 */
		$this->start_controls_section(
			'additional_section',
			array(
		...
		/**
		 * Recipe Posts Carousel Two Content Style Section
		 */
		$this->start_controls_section(
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .dr-recipe-carousel-2' => '--dr-widgetBlock_content-bg: {{VALUE}};',
				),
			)
		);
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .dr-recipe-carousel-2' => '--dr-widgetBlock_content-padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .dr-recipe-carousel-2' => '--recipe-margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'size_units' => array( 'px', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .dr-recipe-carousel-2' => '--recipe-border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		$this->add_group_control(
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .dr-recipe-carousel-2' => '--recipe-alignment: {{VALUE}};',
				),
			)
		);
		$this->end_controls_section();
		...
		/**
		 * Recipe Posts Carousel Two Separator Style Section
		 */
		$this->start_controls_section(
		...
		/**
		 * Recipe Posts Carousel Two Image Style Section
		 */
		$this->start_controls_section(
			'image_style_section',
			array(
		...
		/**
		 * Recipe Posts Carousel Two Title Style Section
		 */
		$this->start_controls_section(
			'title_style_section',
			array(
		...
		/**
		 * Recipe Posts Carousel Two Recipe Keys Style Section
		 * */
		$this->start_controls_section(
		...
		/**
		 * Recipe Posts Carousel Two Meta Style Section
		 * */
		$this->start_controls_section(
			'meta_style_section',
			array(
		...
		$this->start_controls_section(
			...
			array(
				'label'     => esc_html__( 'Recipe Category', 'dr-widgets-blocks' ),
				'tab'       => \Elementor\Controls_Manager::TAB_STYLE,
				'condition' => array(
					'showCategory' => 'yes',
				),
	...
	protected function render() {
		...
		$per_slide        = 'layout-3' === $layout ? 'auto' : ( isset( $settings['recipesPerSlide'] ) ? absint( $settings['recipesPerSlide'] ) : 3 );
		$per_slide_tablet = 'layout-3' === $layout ? 'auto' : ( isset( $settings['recipesPerSlide_tablet'] ) ? absint( $settings['recipesPerSlide_tablet'] ) : 2 );
		$per_slide_mobile = 'layout-3' === $layout ? 'auto' : ( isset( $settings['recipesPerSlide_mobile'] ) ? absint( $settings['recipesPerSlide_mobile'] ) : 1 );
		$item_gap         = isset( $settings['slideItemGap']['size'] ) ? absint( $settings['slideItemGap']['size'] ) : 20;
		$item_gap_tablet  = isset( $settings['slideItemGap_tablet']['size'] ) ? absint( $settings['slideItemGap_tablet']['size'] ) : 20;
		$item_gap_mobile  = isset( $settings['slideItemGap_mobile']['size'] ) ? absint( $settings['slideItemGap_mobile']['size'] ) : 20;
		$swiper_options   = array(
			'slidesPerView' => $per_slide_mobile,
			'spaceBetween'  => $item_gap_mobile,
			'loop'          => 'layout-3' === $layout ? true : ( isset( $settings['loop'] ) ? $settings['loop'] : true ),
			'centeredSlides'=> 'layout-3' === $layout ? true : false,
		...
		$this->add_render_attribute(
			...
			'class',
			[
				'dr-recipe-carousel-2',
				isset( $settings['layout'] ) && ! empty ( $settings['layout'] ) ? esc_attr( 'dr-recipe-carousel-2--' . $settings['layout'] ) : esc_attr( 'dr-recipe-carousel-2--layout-1' ),
			]
		);
					...
					<div class="swiper-wrapper">
						...
						if ( $recipes_query->have_posts() ) {
							while ( $recipes_query->have_posts() ) {
								...
								dr_widgets_blocks_get_template( 'recipe-carousels-two.php', $data );
							}
							wp_reset_postdata();
						}
						?>
