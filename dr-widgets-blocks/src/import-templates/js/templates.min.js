!function(_){"use strict";let e=null,t;async function l(){let _=await (await fetch("https://demo.wpdelicious.com/widgets-blocks-demo/wp-json/delisho-elementor-templates/v1/patterns/")).json(),e={},t={};for(let l of _)l.meta&&l.meta.required_plugins&&(e[l.id]=l.meta.required_plugins),l.meta&&(t[l.id]="1"===l.meta.freevspro);return[e,t]}function a(_=null){return`
    <div class="cw-template-modal-loader-wrapper">
        ...
        <div class="cw-template-modal-loading-title">${_||delishoAdmin.templatesText.loading}</div>
    </div>
`}function i(){let i=window.elementor.$previewContents;!function t(l){let a='<div id="delisho-cw-layout-btn" class="elementor-add-section-area-button transform-scale">'+delishoAdmin.btnIcon+"</div>",i=_("#tmpl-elementor-add-section"),o=i.text(),n=o.replace('<div class="elementor-add-section-drag-title',a+'<div class="elementor-add-section-drag-title');i.text(n),_(l).on("click",".elementor-editor-element-settings .elementor-editor-element-add",function(){let t=_(this).closest(".elementor-element").data("model-cid");window.elementor.elements.models.length&&_.each(window.elementor.elements.models,function(_,l){t===l.cid&&(e=_)})})}(i),function i(o){o.on("click","#delisho-cw-layout-btn",function(){let i=o.find("body"),n=o.find("html");0==o.find(".cw-template-modal").length&&i.append(`
...
<div class="controls-container">
    <div class="controls ready">
        ...
        `),n.css("overflow","hidden"),o.find(".cw-template-modal-overlay").show(),o.find(".cw-close").on("click",function(){n.css("overflow","auto"),o.find(".cw-template-modal-overlay").fadeOut("fast",function(){o.find(".cw-template-modal-overlay").remove()})}),function i(o){let n=o.find(".cw-content-wrap"),s=o.find(".cw-template-modal-loader-wrapper");n.html(a()),t&&t.abort(),t=_.post(delishoAdmin.ajaxURL,{action:"render_templates_designs"}).done(function(t){n.html(t),function e(t){let l=t.find(".cw-layout-two"),a=t.find(".cw-layout-three"),i=t.find(".cw-pattern-library__design-list"),o=t.find(".cw-layout-block"),n=t.find(".cw-layout-page");t.on("click",".cw-layout-two",function(){i.addClass("column2"),_(this).addClass("active"),a.removeClass("active")}),t.on("click",".cw-layout-three",function(){i.removeClass("column2"),_(this).addClass("active"),l.removeClass("active")}),t.on("click",".cw-layout-block",function(){n.removeClass("active"),_(this).addClass("active")}),t.on("click",".cw-layout-page",function(){o.removeClass("active"),_(this).addClass("active")})}(o),function t(i){i.find(".cw-insert-temp").on("click",function(){let t=_(this).attr("data-slug-id"),o=i.find(".cw-template-modal");l().then(l=>{let n=l[0][t]?.filter(_=>!delishoAdmin.activePlugin.some(e=>e.name===_.name)),s=l[1][t],$=delishoAdmin.isLicenseActive;if(n&&n.length>0){o.append(`
                    ...
                    <div class="cw-plugins-dialog-content">
                        <p> To use this template, please install and activate the required plugins. Once completed, you can return to this page to proceed. </p> 
                            <ol class="plugins-list">
                                ${n.map(_=>"Delisho Pro"===_.name?`<li>
                                        ${_.name} - <a class="plugins-link" href="https://wpdelicious.com/delisho/?utm_source=delisho&utm_medium=elementor_button&utm_campaign=upgrade_delisho#pricing" target="_blank"> Get it here</a>
                                        </li>`:`<li>
                                        ${_.name} - <a class="plugins-link" href="${delishoAdmin.url}/wp-admin/plugin-install.php?s=${_.slug}&tab=search&type=term" target="_blank">Install here</a>
                                        </li>`).join("")} 
                            </ol>
                            <span>
            ...
            `),o.on("click","#cw-plugins-dialog-close",function(){o.find(".cw-plugins-dialog").remove()}),o.on("click",'a[target="_blank"]',function(){window.open(this.href,"_blank")}),o.on("click",".reload-page",function(){location.reload()});return}if(s&&!$){o.append(`
                <dialog class="cw-plugins-dialog">
                    <div class="cw-plugins-dialog-wrapper">
                        <div class="header">
                ...
                `),o.on("click","#cw-plugins-dialog-close",function(){o.find(".cw-plugins-dialog").remove()}),o.on("click",'a[target="_blank"]',function(){window.open(this.href,"_blank")});return}{_(this).attr("disabled",!0);let d=`https://demo.wpdelicious.com/widgets-blocks-demo/wp-json/delisho-elementor-templates/v1/patterns/${t}`;_.ajax({url:delishoAdmin.ajaxURL,type:"POST",data:{action:"process_data_for_import",nonce:delishoAdmin.nonce,apiURL:d},beforeSend:function(){console.groupCollapsed("Inserting Demo."),i.find(".cw-template-modal-body").append(a(`${delishoAdmin.templatesText.stay}`))}}).fail(function(_){let e=_.statusText;console.log(e),i.find(".cw-template-modal-loader-wrapper").remove()}).done(function(_){let t=_.data;window.elementor.getPreviewView().addChildModel(t,{at:e}),window.elementor.panel.$el.find("#elementor-panel-footer-saver-publish button").removeClass("elementor-disabled"),window.elementor.panel.$el.find("#elementor-panel-footer-saver-options button").removeClass("elementor-disabled"),e=null,i.find(".cw-template-modal-loader-wrapper").remove(),i.find(".cw-close").trigger("click")})}})})}(o),function e(t){let l={search:"",category:"",plan:""},a=Array.from(t.find(".cw-pattern-library__design-item"));var i,o=300;let n,s=(i=function(e){l={...l,search:_(e.target).val().toLowerCase()},$()},function _(...e){let t=()=>{clearTimeout(n),i(...e)};clearTimeout(n),n=setTimeout(t,300)});function $(){let e=a;l.search&&(e=e.filter(e=>_(e).data().filterName.toLowerCase().indexOf(l.search)>-1)),l.category&&"all"!==l.category&&(e=e.filter(e=>_(e).data().filter.split(" ").includes(l.category))),l.plan&&"all"!==l.plan&&(e=e.filter(e=>{let t=_(e).data(),a=1===t.filterPlan?"pro":"free";return console.log(t.filterPlan),console.log("demoPlan:",a,"filters.plan:",l.plan),a===l.plan})),a.forEach(e=>{_(e).hide()}),e.forEach(e=>{_(e).show()}),0===e.length?(t.find(".cw-no-results")?.remove(),t.find(".cw-pattern-library__design-list").append(`
...
`)):t.find(".cw-no-results")?.remove()}t.on("keyup","#cw-search-control",s),t.on("click",".cw-cat-item",function(){t.find(".cw-cat-item").removeClass("tab-active"),_(this).addClass("tab-active"),l={...l,category:_(this).attr("data-filter")},$()}),t.on("change",".demo-list_dropdown",function(){l={...l,plan:_(this).val()},$()})}(o),function e(t){let l=t.find(".cw-preview-wrap");t.on("click",".cw-pattern-library__design-preview-btn",function(){var e;let i=_(this).attr("data-slug-id");t.find(".cw-header .cw-insert-temp").attr("data-slug-id",i);let o=_(this).attr("data-preview-url");t.find(".cw-content-wrap").hide(),t.find(".cw-template-modal").addClass("preview-active"),l.css("--cw-header-height",t.find(".cw-header").outerHeight()+"px"),l.append(a()),l.append((e=o,`
            <iframe id="cw-template-preview" loading="lazy" allow="fullscreen" style="border-style: none;width: 100%; height: 100%; transition: 0.3s; border-radius: 8px"  src=${e} >
            </iframe>
        `)),l.find("#cw-template-preview").on("load",function(){l.find(".cw-template-modal-loader-wrapper").remove()})}),function e(t){t.on("change",'input[name="template-preivew-group"]',function(){t.find(".cw-preview-wrap").css("--iframe-width",_(this).val())})}(t),function _(e){e.on("click",".cw-header-back-btn",function(){e.find(".cw-content-wrap").show(),e.find(".cw-template-modal").removeClass("preview-active"),e.find(".cw-preview-wrap").empty(),e.find(".cw-template-modal-loader-wrapper").remove()})}(t)}(o),s.remove()}).fail(function(_,e,t){if("abort"===_.statusText){console.log("AJAX request aborted");return}s.remove(),n.html('<div class="cw-error">Error: '+t+"</div>"),console.error("AJAX request failed:",e,t)})}(o)})}(i)}_(window).on("elementor:init",function _(){window.elementor&&window.elementor.on("preview:loaded",i)})}(jQuery);